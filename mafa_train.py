...
from data import AnnotationTransform, VOCDetection, MafaDetection,detection_collate, preproc, cfg
...
rgb_mean = (134, 125, 122) # bgr order
num_classes = 3 #background mask and face
num_gpu = args.ngpu
...
def train():
    ...
    dataset = MafaDetection(training_dataset, preproc(img_dim, rgb_mean), None)
    ...
    for iteration in range(start_iter, max_iter):
        if iteration % epoch_size == 0:
            ...
            if (epoch % 10 == 0 and epoch > 0) or (epoch % 5 == 0 and epoch > 200):
                torch.save(net.state_dict(), save_folder + 'Mafa_FaceBoxes_epoch_' + str(epoch) + '.pth')
            epoch += 1
    ...
    torch.save(net.state_dict(), save_folder + 'Final_Mafa_FaceBoxes.pth')
